...
Є Wi-Fi роутер чи ADSL модем чи будь який інший малопотужний пристрій до 5 Вт. Навіть Led ленти освітлення можна в список додати. Для тесту є декілька пристроїв: Роутери TP-Link, Keenetic, медіконвертори і т.д. з Живленням 12В. Середнє споживання ~250-400 mA. Задача забезпечити можливість автономної роботи (ну як же при work from home і без інтернету) 12 вольтових пристороїв при відсутності вхідної напруги (мої оператори УкрТелеКом і X-My.Net функціонують при відсутності напруги в будинку).
...
Як каже програмістська мудрість працює не рухай (якщо ненаписав юніт-тестів :-) ). Тому було використно вже робочу схему з різницею що MT3608 налаштувати на 12В а не 5В. Схема, фото і елементна база, така сама як і "Версія 0.8", тому без неї. Єдине з акцентую БЖ на вході як і раніше 5В, бо TP4056 іншого не їсть.
...
Cons: Не ефективне перетворення з 5 в 12 В, великі струми, потужний БЖ. На тому експеременти з тим модулем завершились.



### Версія #0.1

#### Контроллер TP5100 і FDC-2S-2 і MT3608 і Діоди Шотки
...
Один з перших що попав на очі і досить таки адекватним є огляд https://mysku.ru/blog/china-stores/80930.html Правда там ставлять під сумнів доцільність струму заряду в 2А. І згадують за цікаві глюки при роботі з одним акумулятором. В мому варіанті зразу випаяв один резистор. На платі в паралель стоїть два по 0.1 Ом (паралельний опір 0,05 Ом) що відповідає 2А струму зарядки. Один резистор забезпечить 1А струм заряду. Для двох 3000 mAh акумів це приблизно має бути 3 години, що є досить нормально. І замкнув перемичку що відповідає за 2S режим (8.4 В)
...
На відміну від TP4056 ця TP5100 немає на борту BMS контролера. На Аліекспресі BMS 2S є два вагона варіантів. Оглядів в неті придостатньо, але зазвичай огляди читаються вже після винекнення якихось проблем. Я взяв одну з найдешевших і розрахованих на відносно низький струм (3 А) плату з маркуванням FDC-2S-2. 

![](FDC-2S-2-photo.jpg)

Для старту можна почитати огляд (правда з купою граматичних помилок) на https://mysku.ru/blog/aliexpress/49071.html Але я звичайно його читав вже після того як при підключенні акумуляторів плата не хотіла видавати на виході напругу. При виборі цієї BMS керувався лише критеріями мінімальна ціна і забезпечує необхідний струм. Коли бавився з тою платою відкрив для себе один нюанс на який раніше ніколи не звертав уваги. Мені просто везло і там де треба попадала необхідна версія BMS. Так от різні BMS можуть бути задизайнені під різні режими поведінки: 1) Для "розблоковування" BMS (при підєднанні нових акумуляторів, після короткого замикання чи захисту від перерозряду батареї) їм необхідно подати на вхід напругу; 2) BMS які само розблоковуються через якийсь час (0.1-1 сек) після короткого замикання чи спрацювання захисту по перерозряду акумуляторів.
Так от тип 1 не годиться для всяких ліхтариків чи шуроповертів. Я переробляв два "шуріка" на літій і там часто буває що при старті або при зупинці шпинделя BMS іде в захист. І якщо би треба було подавати живлення (включати на зарядку) для розблоковки то це би був ад. Там потрібно 2-ий тип. Так от ця плата якраз першого типу. А це дає в схемі переваги: немає "качель" які згадувались в TP4056. Якщо батарея розрядилась, то все, плата повторно стартане лише коли появиться вхідна напруга. Для режиму UPS саме то що треба. На фото видно, напруга на батареї є, а на виході BMS понулях.

![](BMS-locked.jpg)

В якості живлення взяв БЖ 12В, теоретично і 9В мав би працювати, але тоді БЖ треба буде на струм хоча б 2 А. MT3608 налаштована на 12 В на виході. І по аналогії з ["Версія 0.8"](Li-Ion-UPS-18650#версія-08) взяв два діода шоткі. Брав ті самі 1N5817. Як все виглядало на схемі

![](TP5100-BMS-MT3608-Diods.jpg)

#### Тестування

Простенька схема, швиденько заімплементав. Для тестування було використано два Li-Ion акумулятори Litokala, з ємністю 3000мАг. Батареї майже заряджені. Цього разу дожився до активного регульованого навантаження яке виставив на 400 mA. Від акумуляторів схема не завелась, бо BMS було заблоковане. БЖ китайський безкорпусний знятий з якогось обладнання. По характеристиках 12 В і 1 А але як на диво самоблокувався аж при 1.4 А. Макет виглядає якось так:

![](TP5100-BMS-MT3608-Diods-photo.jpg)

Підєднуєм живлення, ждем пару хвилин і плата засигналізувала що акумулятор повністю зарядився (на платі є свої впаяні світлодіоди). На акумуляторах мультиметр показує 8.37 В - майже ідеально. Починаєм експеримент. Від'єднуєм вхідне живлення. І бах напруга на батареях зразу просідає до 8.05В, якось дивно і не зрозуміло (повторив експеремент декілька разів і той самий ефект). Спробую при нагоді після відключення дозарядити адекватною зарядкою, щоб знати скільки бракує. При переході на живлення від батарей явного пропадання напруги не спостерігається та і звідки йому взятись, діоди спрацьовують моментально, тому надіюсь що роутер в ребут не піде. 

До відключення система пропрацювала трохи більше 3 годин і віддала сумарно 15 Вт (12 В * 0.4 А * 3 год), замість ідеальних 22 Вт (3000 * 7.4 В) - КПД перетворювача MT3608, можливий недозаряд батареї, втрати на діоді шотки 0.5 В * 0.4 А * 3 год. Як на мене результат хороший. Батареї відключились на 5.8 В. Зразу після відключення напруга на батареях відновилась до 6.1 В, але BMS не включилась, що мене дуже потішило на відміну від TP4056 де були багаторазові качелі з включенням - виключенням. Діод шотки який підєднує батареї і плату MT3608 був досить гарячим. По розрахункам в кінці роботи, при напрузі на батареї 6 В, струм перед Stepup перетворювачем досягати може 0.9 А а розсіювана потужність до 0.5 Вт.

Відновлюєм вхідне живлення. Не зміг перевірити поведінку контролера вякщо акумулятори розряджені нижче ніж 3В (схема не допустила такого, на 5.8 / 2 = 2.9 В відрубала акумулятори, а вони швиденько відновились до 6.1 / 2 = 3.05 В). Якщо нижче 3В то треба початково заряджати невеликим постійним струмом 100мА до напруги 3В. Цей кейс варто пропрацювати окремо. Зарядка пішла по схемі CC з струмом зарядки 1 А і напруга поступово піднімалась з 6 В до 8.4 В. Плата грілась але руку тримати можна, десь 60 градусів що дуже навіть нормально. Тепер вже грівся інший діод - який прокидує струм з входу на МТ3608, але він значно менше грівся бо і розсіювана потужність була менша завдяки меншим струмам. Споживання входу доходило до 1.2А що є на межі для існуючого в мене БЖ (12 В, 1 А). БЖ досить сильно грівся і нагрівав корпус. Однозначно треба брати 1.5 А або навіть 2 А. При досягненні 8.3 В струм почав зменшуватись і зарядка іти по схемі CV і при 8,37 В плата BMS відключила зарядку, а TP5100 адекватно просигналізувала про завершення зарядки. Тривалість зарядки орієнтовно 3.5 год.

Pros: Повністю працююче згідно вимог рішення. Може бути і з БЖ 9 В. Можна використати stepdown і буде схема для 5В.

Cons: Для ідеального рішення ще планую позбутись діодів і замінити їх на MOSFET. Шукаю шарящого для консультації. Плата TP5100 має цікавий вихід POWER_ON- який можна для того і задіяти.



### Версія #0.2

#### Контроллер TP5100 і FDC-2S-2 і MT3608 і Діоди Шотки і P-MOSFET 

Як не крути а на діодах Шотки всетаки 0б4 в губиться. При роботі від мережі втрати не критичні - для роутера 0,5 А А от діод від батарей гріється сильгіше, особливо коли акумулятори розряжені. І в кінці розраду при напрузі на двох акумуляторах 6В для отримання 12В і 0,5А треба вже струм більше 1А. Також памятаєм що ще 0,4 впаде на діоді. Та і шкода втрачати навіть 10% від ємності акумуляторів. Схемуз двома діодами розширено P-канальним MOSFET транзистором. Діод лишаєм щоб перемикаммя відбулось моментально.

![](Diods-plus-MOSFET.jpg)
